...
*       additional information different steps are taken.
          if lo_gcts_feature_toggles->get_relation_ident_inf_toggles(  )-level = co_level_error and lines( set_repository_response-unresolved_objects ) > 0.
            ...
            message |Error : Not all objects have been resolved to a repository.| type 'E' raising cancel.
          ...
          loop at unique_repositories into data(ls_repository).
            ...
            try.
                ...
                if lv_target_check = abap_false.
                  ...
                  logger->flush(  ).
                  logger->flush( abap_true ).
                  message |Target system of the transport request and the vSID of the repository { ls_repository-rid } do not match| type 'E' raising cancel.
                endif.
                ...
                logger->flush(  ).
                logger->flush( abap_true ).
                message |The target system check failed due to an exception, check Application Logs in SLG1| type 'E' raising cancel.
            endtry.
            try.
                ...
                logger->flush(  ).
                logger->flush( abap_true ).
                message |Git user cannot be authenticated, push not possible| type 'E' raising cancel.
            ...
            try.
                ...
                logger->flush(  ).
                logger->flush( abap_true ).
                message |Write permission missing for repository { ls_repository-rid } locally | type 'E' raising cancel.
                ...
                logger->flush(  ).
                logger->flush( abap_true ).
                message |Cannot release task. Write permission for remote repository { ls_repository-rid } required| type 'E' raising cancel.
            endtry.
*         Do the extra repository evaluation based on the other triggers available.
            ...
            try.
                ...
                logger->flush(  ).
                logger->flush( abap_true ).
                message |System info cannot be retrieved| type 'E' raising cancel.
            endtry.
...
*         Switch or create branch based on the condition set in the maintenance view
              ...
              if branch_change_successful = abap_false.
                ...
                logger->flush(  ).
                logger->flush( abap_true ).
                message |Repository branch handling failed for { ls_repository-rid }| type 'E' raising cancel.
              endif.
            endif.
            try.
                ...
                message | { exc->message_variable_1 } for repository { ls_repository-rid } | type 'E' raising cancel.
            endtry.
            if eval_result = abap_false.
              ...
              logger->flush(  ).
              logger->flush( abap_true ).
              message |Repository evaluation failed for { ls_repository-rid }| type 'E' raising cancel.
            else.
...
*         These repositories would be used to push the objects in the Transport request
            ...
            if not ls_response-exception is initial.
              ...
              logger->flush(  ).
              logger->flush( abap_true ).
              message |Error while committing changes| type 'E' raising cancel.
            endif.
          else.
